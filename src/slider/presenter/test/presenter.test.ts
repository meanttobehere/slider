...
import { ViewObserver, ViewProps } from "../../view/viewInterface";
...
describe("Presenter", () => {
    ...
    beforeEach(() => {
        ...
        viewObserver = view.setObserver.calls.argsFor(0)[0];         
        modelObserver = modelSpy.calls.argsFor(0)[0];        
    });    
    ...
    it("Presenter getUpdateFunction method returns a function to update model", () => {
        let update = presenter.getUpdateFunction();        
        let modelData: ModelData = {
            typeVertical: false,
            typeRange: false,
            displayTips: false,
        ...
        update({pointerPosition: -10, minValue: -20, secondPointerPosition: 30, maxValue: 50});
        expect(model.getData()).toEqual({...modelData, ...{pointerPosition: -10, minValue: -20, secondPointerPosition: 30, maxValue: 50}});
    });

    it("Presenter getSetters add getGetters methods work correctly", () => {
        let set = presenter.getSetters();
        let get = presenter.getGetters();

        set.minValue(-100);
        expect(get.minValue()).toEqual(-100);
